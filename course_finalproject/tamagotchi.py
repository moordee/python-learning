...
AGE_HATCH = 128                      
AGE_MATURE = 496                     
AGE_RESET_THRESHOLD = 1000          
AGE_DEATHFROMNATURALCAUSES =1000    
HUNGER_CANEAT = 32                   
...
def render_display(image_data, fg_color, bg_color, off=0, percentBar=0):
    for y in range(32):  
        ...
        for x in range(off, 32+off):  
            ...
            if x in range(len(bits)):
                if bits[x] or percentBar > 0 and y > 11 and x > 2 and y < 17 and x < 3 + percentBar:
                    color = fg_color
...
def main():
    ...
    while True:
        ...
        if stats:
            ...
            if startpageid == 0:
                percentBar = pet['hunger'] * 27 // HUNGER_NEEDSTOEAT
            elif startpageid == 1:
                percentBar = pet['age'] * 27 // AGE_RESET_THRESHOLD 
            elif startpageid == 2:
                percentBar = (pet['waste'] % WASTE_EXPUNGE) * 27 // WASTE_EXPUNGE
            elif startpageid == 3:
                percentBar = pet['energy'] * 27 // 256
            elif startpageid == 4:
                percentBar = 0
            ...
            render_display(stats_page, PIXEL_COLOR, NONPIXEL_COLOR, 0, percentBar)
        else:
